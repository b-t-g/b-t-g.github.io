...
you can take that we require a morphism from $\eta_X:F(X)\to G(X)$, which is to say that we need $F(X)$ and $G(X)$ to have the same structure (whatever that means in our category of choice).
The bottom arrow is a similar story: If I have some
