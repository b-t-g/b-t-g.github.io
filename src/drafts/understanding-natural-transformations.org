...
a "category homomorphism" (functor) sends categories to categories, etc. In fact, in a general category $C$, for two objects $X,Y\in ob(C)$[fn:1], a morphism $f:X\to Y$ means
that $Y$ has the same structure as $X$. Even though that last point sounds like a needless abstraction, that insight will be useful later.
...
we require a morphism from $\eta_X:F(X)\to G(X)$, which is to say that we need $F(X)$ and $G(X)$ to have the same structure (whatever that means in our category of choice).

The bottom arrow is surprisingly deep. If we stopped here, we would have the following diagram:
\[\begin{CD}
F(X) @>\eta_X>> G(X) \\
@VF(f)VV        @VVG(f)V \\
F(Y) @.         G(Y)
...
Now what about morphisms being preserved? Well it turns out that we get that for free! From [[https://en.wikipedia.org/wiki/Functor#Definition][the definition of a functor]], for a functor $F$, if we have $f\in mor(X,Y)$, we automatically get a morphism
$F(f)\in mor(F(X), F(Y))$[fn:3].
...
A chain complex $C_\bullet$ (or just $C$) is a collection of $R-\text{modules}$[fn:4] $C_i$ along with module homomorphisms $d_i: C_i\to C_{i-1}$ such that $d_{i-1}\circ d_{i} = 0$
...
[fn:3] Assuming $F$ is a [[https://en.wikipedia.org/wiki/Functor#Covariance_and_contravariance][covariant functor]].
[fn:4] If you aren't familiar with modules, replace all instances with "R-module" with "vector space".
