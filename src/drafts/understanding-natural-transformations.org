...
a "category homomorphism" (functor) sends categories to categories, etc. More generally, for an arbitrary category $C$, if, for two objects $X,Y\in ob(C)$[fn:1], there is a morphism $f:X\to Y$
...
With this in mind, what is the structure of a functor? It's a strange question with a straightforward answer: you apply it. You give me an object from a category $C$,
