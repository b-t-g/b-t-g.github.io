...
$\eta_{\mathbb{Z}\slash2\mathbb{Z}}$ to be the zero function. If we didn't specify the bottom arrow, the resulting in the non-commutative diagram would represent a natural transformation.

\[\begin{CD}
\mathbb{Z}                   @>x\mapsto x>> \mathbb{Z} \\
@Vx\mapsto x\mod 2VV                        @VVx\mapsto x\mod 2V \\
\mathbb{Z}\slash 2\mathbb{Z} @>0>>          \mathbb{Z}\slash 2\mathbb{Z}
\end{CD} \]
